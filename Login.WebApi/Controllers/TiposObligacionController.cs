...
public class TiposObligacionController : ControllerBase
{
    private readonly DataContext _db;
    public TiposObligacionController(DataContext db) => _db = db;

    [HttpGet]
    public async Task<ActionResult<IEnumerable<ParametricDto>>> GetAll()
        => Ok(await _db.TiposObligacion
    ...
    {
        var x = await _db.TiposObligacion.FindAsync(id);
    ...
    public async Task<ActionResult<ParametricDto>> Create(CreateParametricRequest req)
    {
        ...
        if (await _db.TiposObligacion.AnyAsync(x => x.Name.ToLower() == name.ToLower()))
            return Conflict(new { message = "Ya existe un registro con ese nombre" });

        var entity = new TipoObligacion
        {
            Name = name,
        ...
        _db.TiposObligacion.Add(entity);
    ...
    {
        var entity = await _db.TiposObligacion.FindAsync(id);
        if (entity is null) return NotFound();

        var name = req.Name.Trim();
        if (await _db.TiposObligacion.AnyAsync(x => x.Id != id && x.Name.ToLower() == name.ToLower()))
            return Conflict(new { message = "Ya existe un registro con ese nombre" });
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        _db.TiposObligacion.Remove(entity);
        await _db.SaveChangesAsync();
        return NoContent();
    }
}
