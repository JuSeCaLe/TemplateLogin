...
public class TiposProcesoController : ControllerBase
{
    ...
    public TiposProcesoController(DataContext db) => _db = db;

    [HttpGet]
    public async Task<ActionResult<IEnumerable<ParametricDto>>> GetAll()
        => Ok(await _db.TiposProceso
    ...
    {
        var x = await _db.TiposProceso.FindAsync(id);
    ...
    public async Task<ActionResult<ParametricDto>> Create(CreateParametricRequest req)
    {
        ...
        if (await _db.TiposProceso.AnyAsync(x => x.Name.ToLower() == name.ToLower()))
            return Conflict(new { message = "Ya existe un registro con ese nombre" });

        var entity = new TipoProceso
        {
            Name = name,
    ...
    {
        var entity = await _db.TiposProceso.FindAsync(id);
        if (entity is null) return NotFound();

        var name = req.Name.Trim();
        if (await _db.TiposProceso.AnyAsync(x => x.Id != id && x.Name.ToLower() == name.ToLower()))
            return Conflict(new { message = "Ya existe un registro con ese nombre" });
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        _db.TiposProceso.Remove(entity);
        await _db.SaveChangesAsync();
        return NoContent();
    }
}
